...
import CompanyUserDetails from '../CompanyUserDetail';
...
describe("CompanyUserDetails component", () => {
  test("check renders", () => {
    const wrapper = shallow(<CompanyUserDetails modal={false} userDetails={[]} title={""} />);
  ...
  it('should call componentWillReceiveProps', () => {
    ...
    //update the instance with the new spy
    wrapper.instance().componentWillReceiveProps({ userDetails: [], modal: true });
  ...
  it('should call toggle', () => {
    ...
    const wrapper = shallow(<CompanyUserDetails updateState={clickFn} modal={false} userDetails={[]} title={""} />);
    wrapper.instance().toggle();
    expect(clickFn).toHaveBeenCalled();
    wrapper.unmount();
  });
  
  it('should call async getUserTaskDetails', () => {
    const wrapper = shallow(<CompanyUserDetails />);
    const spy = jest.spyOn(wrapper.instance(), 'getUserTaskDetails');
    //update the instance with the new spy
    wrapper.instance().getUserTaskDetails("", 1, 1, false, 1);
    //invoke getUserTaskDetails
    expect(spy).toBeCalled();
  });
  ...
  it('should call handleCellClick', () => {
    ...
    //update the instance with the new spy
    wrapper.instance().handleCellClick("completed", args);
    wrapper.instance().handleCellClick("incomplete", args);
