...
namespace ReportBuilderAPI.Repository
{
    ...
    public class WorkbookRepository : IWorkbook
    {
        ...
        private readonly Dictionary<string, string> workbookColumnList = new Dictionary<string, string>()
        {
                ...
                { Constants.TOTAL_TASK, ", (SELECT ISNULL((SELECT COUNT(wbt.EntityId) FROM  dbo.WorkBookContent wbt JOIN dbo.UserWorkBook uwb ON uwb.workbookId=wbt.WorkbookId AND uwb.IsEnabled=1 JOIN dbo.Supervisor s ON uwb.UserId = s.UserId AND s.UserId IN(SELECT u.User_Id  UNION SELECT UserID FROM Supervisor WHERE SupervisorId=u.User_Id  AND SupervisorId <> @currentuserId) AND s.IsEnabled = 1  JOIN dbo.UserCompany uc on uc.UserId= s.UserId   AND uc.IsEnabled = 1 AND uc.Status = 1 AND uc.IsVisible = 1  WHERE uc.companyId=@companyId AND wbt.IsEnabled=1 Group by wbt.workbookId),0)) AS TotalTasks"},
