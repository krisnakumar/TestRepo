...
namespace ReportBuilderAPI.Repository
{
    ...
    public class WorkbookRepository : IWorkbook
    {
        ...
        private readonly Dictionary<string, string> workbookColumnList = new Dictionary<string, string>()
        {
                ...
                { Constants.ASSIGNED_WORKBOOK, ", (SELECT COUNT(uwt.WorkBookId) AS WorkbookCount FROM dbo.UserWorkBook uwt   INNER  JOIN dbo.UserCompany uc ON uc.UserId= uwt.UserId AND uc.IsEnabled = 1 AND uc.Status = 1 AND uc.IsVisible = 1  WHERE uc.UserId In (384658) AND uc.companyId=2288 AND uwt.IsEnabled=1  )  AS AssignedWorkbooks"},
