...
namespace ReportBuilderAPI.Repository
{
    ...
    public class WorkbookRepository : IWorkbook
    {
        ...
        private readonly Dictionary<string, string> workbookColumnList = new Dictionary<string, string>()
        {
                ...
                { Constants.ASSIGNED_WORKBOOK, ", (SELECT SUM(WorkbookCount) FROM (SELECT COUNT(uwt.WorkBookId) AS WorkbookCount FROM dbo.UserWorkBook uwt   INNER  JOIN dbo.UserCompany uc ON uc.UserId= uwt.UserId AND uc.IsEnabled = 1 AND uc.Status = 1 AND uc.IsVisible = 1  INNER JOIN dbo.Supervisor s ON uc.UserId = s.UserId AND s.UserId IN (384658) AND s.IsEnabled = 1 WHERE uc.companyId=@companyId AND uwt.IsEnabled=1  ) AS AssignedWorkbooks) AS AssignedWorkbooks"},
