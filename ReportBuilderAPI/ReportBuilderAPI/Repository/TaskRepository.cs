    ...
    public class TaskRepository : ITask
    {
        ...
        public string CreateTaskQuery(QueryBuilderRequest queryBuilderRequest)
        {
            ...
            try
            {
                ...
                {
                    whereQuery += "AND u.Id IN (SELECT u.Id FROM dbo.[User] u JOIN dbo.userCompany uc ON uc.UserId = u.Id JOIN dbo.UserRole ur ON ur.UserId = u.Id JOIN dbo.Role r ON r.Id = ur.roleId WHERE r.Name = 'SUPERVISOR' and uc.companyId = @companyId))";
                }
                query += whereQuery;
                return query;
            }
        ...
        private readonly Dictionary<string, string> taskColumnList = new Dictionary<string, string>()
        {
                ...
                { Constants.IN_COMPLETE_QUALIFICATION,",  (SELECT ISNULL((SELECT COUNT(DISTINCT taskId) FROM dbo.TranscriptSkillsDN ts WHERE  Knowledge_Cert_Status IN (3, 2,0,4, 255) AND  ts.User_Id IN (SELECT u.Id UNION SELECT * FROM getchildUsers(u.Id)) AND ts.CompanyId=@companyId ) ,0)) AS IncompleteQualification" },
                { Constants.PAST_DUE_QUALIFICATION,", (SELECT ISNULL((SELECT COUNT(DISTINCT taskversionId) FROM dbo.courseAssignment cs  WHERE UserId In (SELECT u.id UNION SELECT * FROM getchildUsers( u.id)) AND cs.IsEnabled = 1 AND cs.Status = 0 AND cs.IsCurrent = 1 AND cs.ExpirationDate < GETDATE() AND  ABS(DATEDIFF(DAY,GETDATE(),cs.ExpirationDate))<=@duedays AND cs.CompanyId=@companyId AND cs.FirstAccessed IS NULL ) ,0)) AS PastDueQualification" },
                { Constants.IN_DUE_QUALIFICATION,", (SELECT ISNULL((SELECT COUNT(DISTINCT taskversionId) FROM dbo.courseAssignment cs  WHERE UserId In (SELECT u.id UNION SELECT * FROM getchildUsers(u.id)) AND cs.IsEnabled = 1 AND cs.Status = 0 AND cs.IsCurrent = 1 AND cs.ExpirationDate > GETDATE() AND  ABS(DATEDIFF(DAY,GETDATE(),cs.ExpirationDate))<=@duedays AND cs.CompanyId=@companyId AND cs.FirstAccessed IS NULL ) ,0)) AS InDueQualification" },
        ...
        private readonly Dictionary<string, List<string>> tableJoins = new Dictionary<string, List<string>>()
        {
             ...
             { " LEFT JOIN dbo.TaskSkill ts ON ts.TaskversionId=tv.Id 	 LEFT JOIN dbo.CourseAssignment ca ON ca.taskversionId=ts.taskversionId  	  LEFT JOIN dbo.Course c on c.Id=ca.courseId LEFT JOIN dbo.transcriptSkillsDN tss on tss.taskId=t.Id       FULL OUTER JOIN dbo.userCompanySeries ucs ON ucs.userId=ca.userId  LEFT JOIN  dbo.Usercompany uc on uc.userId=ucs.userId    FULL OUTER JOIN dbo.[User] u ON u.Id=ucs.UserId FULL OUTER JOIN dbo.Supervisor s ON s.userId=u.Id    LEFT JOIN dbo.Company cy ON cy.Id=ucs.CompanyId  FULL OUTER JOIN dbo.UserRole ur ON ur.userId=u.Id FULL OUTER JOIN dbo.Role r ON r.Id=ur.RoleId " , new List<string> {Constants.ASSIGNED_QUALIFICATION, Constants.COMPLETED_QUALIFICATION, Constants.IN_DUE_QUALIFICATION, Constants.PAST_DUE_QUALIFICATION, Constants.IN_COMPLETE_QUALIFICATION, Constants.EMPLOYEE_NAME , Constants.ASSIGNED_DATE, Constants.COURSE_EXPIRATION_DATE, Constants.ASSIGNED_COMPANY_QUALIFICATION, Constants.COMPLETED_COMPANY_QUALIFICATION, Constants.IN_COMPLETE_COMPANY_QUALIFICATION, Constants.PAST_DUE_COMPANY_QUALIFICATION, Constants.IN_DUE_COMPANY_QUALIFICATION, Constants.TOTAL_COMPANY_EMPLOYEES} }
        };
    }
}
