    ...
    public class TaskRepository : ITask
    {
        ...
        private readonly Dictionary<string, List<string>> tableJoins = new Dictionary<string, List<string>>()
        {
             { " FROM dbo.Task t LEFT JOIN dbo.TaskVersion tv ON tv.TaskId=t.Id  LEFT JOIN dbo.TaskSkill ts ON ts.TaskVersionId=tv.Id LEFT JOIN dbo.SkillActivity sa ON sa.SkillId=ts.SkillId    LEFT JOIN dbo.SCOStatus ss ON ss.status=sa.Status LEFT JOIN dbo.SkillActivityMetrics sam ON sam.SkillActivityId=sa.Id   LEFT JOIN dbo.WorkBookContent wbc ON wbc.EntityId=t.Id    LEFT JOIN dbo.UserWorkBook uwb ON uwb.WorkbookId=wbc.WorkbookId   LEFT JOIN dbo.[UserDetails_RB] u on u.User_Id=uwb.UserId LEFT JOIN dbo.Usercompany uc on uc.userId=u.User_Id AND uc.IsDefault=1 AND uc.IsEnabled=1" , new List<string> {Constants.DATE_EXPIRED, Constants.DATE_TAKEN, Constants.CITY, Constants.STATE, Constants.ZIP, Constants.COUNTRY, Constants.IP, Constants.SCORE, Constants.DURATION, Constants.EVALUATOR_NAME, Constants.ASSIGNED_TO, Constants.COMPLETION_DATE, Constants.LAST_ATTEMPT_DATE, Constants.NUMBER_OF_ATTEMPTS, Constants.EXPIRATION_DATE, Constants.EVALUATOR_NAME , Constants.CREATED_BY, Constants.DELETED_BY, Constants.WORKBOOK_ID, Constants.EVALUATOR_NAME, Constants.STATUS } },
              ...
              { " FROM dbo.Task t LEFT JOIN dbo.TaskVersion tv ON tv.TaskId=t.Id  JOIN dbo.CourseAssignment ca on ca.TaskversionId=tv.Id   JOIN  dbo.UserRole ur on ur.UserId=ca.UserId   JOIN dbo.Role r on r.Id=ur.roleId   JOIN dbo.UserCompany uc on uc.UserId=ur.UserId    AND  uc.IsEnabled = 1 AND uc.Status = 1 AND uc.IsVisible = 1 AND uc.IsDefault=1 LEFT JOIN dbo.[UserDetails_RB] u on u.User_Id=uc.UserId  LEFT JOIN dbo.CompanyClient cc ON uc.CompanyId=cc.OwnerCompany AND cc.IsEnabled=1 AND cc.ClientCompany!=uc.CompanyId   JOIN dbo.Company cy on cy.Id=uc.CompanyId AND cy.IsEnabled=1 " , new List<string> {Constants.COMPLETED_COMPANY_USERS, Constants.NOT_COMPLETED_COMPANY_USERS, Constants.TOTAL_COMPLETED_COMPANY_USERS, Constants.EMPLOYEE_NAME, Constants.ASSIGNED_DATE,Constants.TASK, Constants.TASK_CODE } },

             { " FROM dbo.Usercompany uc LEFT JOIN dbo.CompanyClient cc ON uc.CompanyId=cc.OwnerCompany AND cc.IsEnabled=1 AND cc.ClientCompany!=uc.CompanyId AND  uc.IsEnabled = 1 AND uc.Status = 1 AND uc.IsVisible = 1 AND uc.IsDefault=1 FULL OUTER JOIN dbo.[UserDetails_RB] u ON u.User_Id=uc.UserId FULL OUTER JOIN dbo.Supervisor s ON s.userId=u.User_Id    LEFT JOIN dbo.Company cy ON cy.Id=uc.CompanyId AND cy.IsEnabled=1 " , new List<string> {Constants.ASSIGNED_QUALIFICATION, Constants.COMPLETED_QUALIFICATION, Constants.IN_DUE_QUALIFICATION, Constants.PAST_DUE_QUALIFICATION, Constants.IN_COMPLETE_QUALIFICATION,  Constants.COURSE_EXPIRATION_DATE,  Constants.LOCK_OUT_REASON } },

               { "  FROM dbo.DashboardReportDN dr  LEFT JOIN  dbo.Usercompany uc on uc.userId=uc.userId  AND  uc.IsEnabled = 1 AND uc.Status = 1 AND uc.IsVisible = 1 AND uc.IsDefault=1 AND userid=@userId  LEFT JOIN dbo.[UserDetails_RB] u on u.User_Id=uc.UserId  LEFT JOIN dbo.Company cy ON cy.Id=uc.CompanyId AND cy.IsEnabled=1 WHERE dr.User_Id IN (SELECT @userId UNION select sr.userId from Supervisor sr WHERE  sr.SupervisorId=@userId AND sr.IsDirectReport=1 AND sr.IsEnabled = 1) and dr.Company_Id=@companyId GROUP BY u.User_Id,cy.Name, cy.Id" , new List<string> { Constants.ASSIGNED_COMPANY_QUALIFICATION, Constants.COMPLETED_COMPANY_QUALIFICATION, Constants.IN_COMPLETE_COMPANY_QUALIFICATION, Constants.PAST_DUE_COMPANY_QUALIFICATION, Constants.IN_DUE_COMPANY_QUALIFICATION, Constants.TOTAL_COMPANY_EMPLOYEES } },
