    ...
    public class TaskRepository : ITask
    {
        ...
        private readonly Dictionary<string, string> taskColumnList = new Dictionary<string, string>()
        {
                ...
                { Constants.COMPLETED_QUALIFICATION,", (SELECT ISNULL((SELECT COUNT( taskId) FROM dbo.TranscriptSkillsDN ts WHERE Knowledge_Cert_Status = 1 AND Knowledge_Status_Code = 5 AND ts.IsEnabled = 1 AND Date_Knowledge_Cert_Expired > GETDATE() AND ts.User_Id = u.User_Id AND ts.CompanyId=@companyId  ) ,0)) AS  CompletedQualification" },
                 ...
                 { Constants.DISQUALIFIED_QUALIFICATION,", (SELECT ISNULL((SELECT COUNT( taskId) FROM dbo.TranscriptSkillsDN ts WHERE Knowledge_Cert_Status = 2 AND ts.User_Id = u.User_Id AND ts.CompanyId=@companyId AND ts.IsEnabled = 1 ) ,0)) AS  DisqualifiedQualification" },


                 { Constants.SUSPENDED_QUALIFICATION,", (SELECT ISNULL((SELECT COUNT( taskId) FROM dbo.TranscriptSkillsDN ts WHERE Knowledge_Cert_Status = 4 AND ts.User_Id = u.User_Id AND ts.CompanyId=@companyId AND ts.IsEnabled = 1 ) ,0)) AS  SuspendedQualification" },
